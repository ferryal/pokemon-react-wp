...
import { Loading, Scroll } from './components/atoms';
import { Filter, InfoDialog, Pokemon } from './components/organism';
...
class App extends React.Component {
  ...
  getAllPokemons = async (offset, limit) => {
    ...
    getPokemonData = async (result) => {
      ...
      if (this.state.isTypeSelected) {
        for (let i = 0; i < pokemonArr.length; i += 1) {
          for (let j = 0; j < pokemonArr[i].types.length; j += 1) {
    ...
    fetchEvoImages = async (evoChainArr) => {
      for (let i = 0; i < evoChainArr.length; i += 1) {
        const response = await axios
    ...
    fetchPokemonData = async (number, pokemon, category, imageURL) => {
      ...
      for (let i = 0; i < response.data.abilities.length; i += 1) {
        abs.push(response.data.abilities[i].ability.name);
      }

      for (let j = 0; j < response.data.stats.length; j += 1) {
        const Obj = {};
    ...
    fetchPokemonDescription = async (pokemon_name) => {
      ...
      try {
        for (let i = 0; i < response.data.flavor_text_entries.length - 1; i += 1) {
        ...
        for (let j = 0; j < response.data.genera.length; j += 1) {
    ...
    handleChangeRegions = (event) => {
      for (let i = 0; i < this.state.regions.length; i += 1) {
